...
async function route(r: Request,e: Env) {
    ...
    if(p==='/v1/version'&&r.method==='GET'){const release=await settings(e,'release');const latestVersion=release.latest_version||e.LATEST_VERSION||'0.0.0';return json({service:'QLCAD API',latest_version:latestVersion,app_version:latestVersion,worker_build_id:e.WORKER_BUILD_ID||'local',schema_version:e.WORKER_SCHEMA_VERSION||'unknown',deployed_at:e.WORKER_DEPLOYED_AT||null,ai_routing_version:e.AI_ROUTING_VERSION||'router-v1',mandatory:false,release_notes:release.release_notes||'',download_url:release.download_url||'',backup_download_url:release.backup_download_url||'',package_size:Number(release.package_size||0)||null,package_sha256:release.package_sha256||'',package_signature:release.package_signature||'',package_type:release.package_type||'',signing_key_id:release.signing_key_id||''},200,origin);}
