-- ONE-TIME upgrade for the verified legacy orders table (8 columns).
-- Use this file INSTEAD OF migrations/0003_payments.sql on that legacy schema.
-- Does not delete/rename tables or change existing order values or memberships.
-- Re-running aborts on the first duplicate column; do not run on a fresh/new schema.
ALTER TABLE orders ADD COLUMN plan_id TEXT REFERENCES plans(id);
ALTER TABLE orders ADD COLUMN duration_days INTEGER;
ALTER TABLE orders ADD COLUMN membership_level TEXT;
ALTER TABLE orders ADD COLUMN payable_cents INTEGER;
ALTER TABLE orders ADD COLUMN provider TEXT NOT NULL DEFAULT 'ezfpy';
ALTER TABLE orders ADD COLUMN channel TEXT;
ALTER TABLE orders ADD COLUMN provider_trade_no TEXT;
ALTER TABLE orders ADD COLUMN create_state TEXT NOT NULL DEFAULT 'creating';
...
ALTER TABLE orders ADD COLUMN last_error_code TEXT;
-- Existing orders must never be mistaken for newly created, verified payments.
UPDATE orders SET provider='legacy',create_state='legacy';
-- Full unique index matches the checkout ON CONFLICT target; legacy NULL keys coexist.
CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_idempotency ON orders(user_id,idempotency_key);
