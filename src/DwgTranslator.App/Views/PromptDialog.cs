...
public sealed class PromptDialog : Window
{
    ...
    private PromptDialog(string message, string title, MessageBoxButton buttons)
    {
        ...
        if (buttons is MessageBoxButton.YesNo or MessageBoxButton.YesNoCancel) { Add(buttons == MessageBoxButton.YesNoCancel ? "放弃更改" : "否", MessageBoxResult.No); Add(buttons == MessageBoxButton.YesNoCancel ? "保存更改" : "确定", MessageBoxResult.Yes, true); }
    ...
    public static MessageBoxResult Show(string message, string title = "提示", MessageBoxButton buttons = MessageBoxButton.OK, MessageBoxImage icon = MessageBoxImage.None)
    {
        ...
        try { var dialog = new PromptDialog(message, title, buttons) { Owner = owner }; dialog.ShowDialog(); return dialog._result; }
