...
async function pass(p: string, pepper: string) {
  const b = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(p + "\\0" + pepper),
    "PBKDF2",
    false,
    ["deriveBits"],
  );
  const bits = await crypto.subtle.deriveBits(
    {
      name: "PBKDF2",
      salt: new TextEncoder().encode("dwgc2e-password-v1"),
      iterations: 100000,
      hash: "SHA-256",
    },
    b,
...
async function auth(r: Request, e: Env) {
  ...
  const row = await e.DB.prepare(
    "SELECT s.user_id,s.device_id,u.account,u.display_name,u.email,u.is_active,s.expires_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=? AND s.revoked_at IS NULL",
  )
    .bind(await digest(t))
    .first<J>();
...
async function verifyCode(
  ...
  await e.DB.prepare("UPDATE email_verification_codes SET used_at=? WHERE id=?")
    .bind(now(), row.id)
    .run();
  return true;
}
async function glossary(r: Request, e: Env, user: J) {
  ...
  if (!(await verifyCode(email, "password_reset", code, e)))
    return json(
      {
        success: false,
  ...
  const u = await e.DB.prepare("SELECT id FROM users WHERE lower(email)=?")
    ...
    .first<J>();
  if (u) {
    await e.DB.prepare("UPDATE users SET password_hash=? WHERE id=?")
      .bind(await pass(password, e.PASSWORD_PEPPER), u.id)
      .run();
    await e.DB.prepare(
...
async function login(r: Request, e: Env) {
  ...
  const u = await e.DB.prepare("SELECT * FROM users WHERE account=?")
    .bind(String(b.account).trim())
    .first<J>();
  if (
    !u ||
    u.password_hash !== (await pass(String(b.password), e.PASSWORD_PEPPER))
  )
    return json(
      {
        success: false,
  ...
  if (!device || device.length > 200) return json({ success: false, error_code: "invalid_device", message: "设备标识无效" },400,cors(e));
...
async function reserveQuota(e: Env, userId: string, ym: string, chars: number) {
  await e.DB.prepare("INSERT OR IGNORE INTO usage_monthly(user_id,year_month,chars_used,chars_quota,task_count) VALUES(?,?,?,?,0)").bind(userId,ym,0,quota(e)).run();
  const result = await e.DB.prepare("UPDATE usage_monthly SET chars_used=chars_used+? WHERE user_id=? AND year_month=? AND chars_used+?<=chars_quota").bind(chars,userId,ym,chars).run();
  return Number(result.meta?.changes || 0) === 1;
}
async function settleQuota(e: Env, userId: string, ym: string, reserved: number, billed: number) {
  await e.DB.prepare("UPDATE usage_monthly SET chars_used=chars_used-?+?,task_count=task_count+1 WHERE user_id=? AND year_month=?").bind(reserved,billed,userId,ym).run();
}
async function translate(r: Request, e: Env, user: J) {
  ...
  if (clean.some(x => !Number.isInteger(x.id) || !x.text.trim() || x.text.length > Number(e.MAX_TEXT_LENGTH || 2000)) || new Set(clean.map(x => x.id)).size !== clean.length) return json({ success:false, error_code:"invalid_text", message:"文本为空、过长或条目标识重复" },400,cors(e));
  const chars = clean.reduce((n, x) => n + x.text.length, 0);
  const ym = now().slice(0, 7);
  if (!(await reserveQuota(e,user.user_id,ym,chars)))
    return json({success:false,error_code:"quota_exceeded",message:"本月翻译额度不足"},402,cors(e));
  const prompt = `Translate CAD drawing text from ${b.source_lang} to ${b.target_lang}. Return ONLY a JSON array of objects with id and translated_text. Preserve numbers, dimensions, tolerances, model numbers and format. Items: ${JSON.stringify(clean)}`;
  ...
  try {
    response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You translate technical CAD labels accurately.",
          },
    ...
    await settleQuota(e,user.user_id,ym,chars,0);
    return json(
      {
        success: false,
  ...
  let d: J;
  try {
    d = (await response.json()) as J;
  } catch {
    await settleQuota(e, user.user_id, ym, chars, 0);
    return json(
      { success: false, error_code: "upstream_invalid_response", message: "翻译服务返回格式无效" },
      502,
      cors(e),
    );
  }
  ...
  try {
    const parsed = JSON.parse(
      String(d.choices?.[0]?.message?.content || "[]").replace(
        /^```json\s*|```$/g,
        "",
      ),
    );
    ...
    await settleQuota(e, user.user_id, ym, chars, 0);
    return json(
      { success: false, error_code: "upstream_invalid_response", message: "翻译结果格式无效" },
      502,
      cors(e),
    );
  }
  const result = clean.map((x) => {
    const z = out.find((y) => Number(y.id) === x.id);
    return z?.translated_text
      ? {
          id: x.id,
          translated_text: String(z.translated_text),
          from_cache: false,
        }
      : { id: x.id, error_code: "missing_result" };
  });
  const billed = result.reduce((sum,x,i) => sum + (x.translated_text ? clean[i].text.length : 0), 0);
  await settleQuota(e,user.user_id,ym,chars,billed);
  return json(
    { success: true, characters_used: chars, cached_count: 0, items: result },
    200,
    cors(e),
  );
}
export default {
  async fetch(r: Request, e: Env) {
    ...
    if (p === "/v1/usage") {
      ...
      return json(
        {
          monthly_quota: Number(x?.chars_quota || quota(e)),
