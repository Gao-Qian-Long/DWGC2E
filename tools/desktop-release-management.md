...
根目录运行 `release-manager.bat`。发布助手把产品版本、正式构建、安装包投递、更新签名和后台发布串成一条流程：

1. 填写目标公开版本（例如 `2.1.2`），点击“同步版本”。APP 的程序集版本读取 `release/version.props`；若网站仓库位于同级 `DWGC2E_Website`，同时更新 `js/site-config.js` 的离线兜底版本。
2. 点击“构建并生成 upload”。工具继续调用既有完整 `build-setup.bat`，测试/安装验收/回滚保护不被绕过。
3. 把 `upload\QLCAD-<版本>-...-Setup.exe` 上传到网盘。主下载地址必须是可直接返回该 EXE 字节的 HTTPS 直链；分享页 HTML 不能用于 APP 自动更新。
4. 选择与 APP 内置公钥对应的 RSA 私钥 PEM，填写后台管理员密钥、主/备用下载地址和更新说明。“验证下载直链”会完整下载远端文件并与本地 Setup 的大小和 SHA-256 比对。
5. 点击“发布”。工具签名最终 Setup，向后台写入版本、下载链接、说明、包大小、SHA-256、签名、包类型与 Key ID，并回读公开 `/v1/version`。公开接口未完整返回安全元数据时发布助手不会报告成功。

官网与 APP 都以后台发布配置为线上权威来源；`site-config.js` 仅为官网无法读取后台配置时的静态兜底。已经安装在用户电脑中的 APP 版本不会因后台改版本号而自行变化，必须完成更新安装后才会显示新版本。
