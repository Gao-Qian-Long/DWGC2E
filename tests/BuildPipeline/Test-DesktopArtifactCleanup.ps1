...
try {
 foreach($stamp in @('20260914-100000','20260915-100000')){
  ...
  Set-Content -LiteralPath (Join-Path $dir 'QLCAD.exe') 'fixture'
  @{sha256=(Get-FileHash -LiteralPath (Join-Path $dir 'QLCAD.exe')).Hash;version="2.1.1+ui.$stamp.test"}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $dir 'build-info.json')
  Set-Content -LiteralPath (Join-Path $art "QLCAD-win-x64-GstarCAD-$stamp.zip") 'fixture zip'
 }
 ...
 foreach($name in @('publish-20260915-100000','publish-20260913-100000','release-backup-20260914-100000','QLCAD-win-x64-GstarCAD-20260914-100000.zip','QLCAD-win-x64-GstarCAD-20260915-100000.zip','payment-backup.sql')){Check (Test-Path -LiteralPath (Join-Path $art $name)) "Protected artifact missing: $name"}
