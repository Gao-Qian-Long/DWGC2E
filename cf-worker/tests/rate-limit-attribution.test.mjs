...
test('login IP bucket ignores spoofed headers without weakening account limiter',async t=>{
 ...
 for(let n=0;n<20;n++)assert.equal((await call('no-such-account','203.0.113.1','198.51.100.'+n)).status,401);
 ...
 assert.equal(x.db.prepare("SELECT count FROM request_limits WHERE key LIKE 'login-ip:%'").get().count,20);
