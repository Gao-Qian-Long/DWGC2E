...
if((Get-FileHash -LiteralPath (Join-Path $publish 'QLCAD.exe')).Hash -ne $info.sha256){throw 'Unverified candidate executable'}
...
function Compile([string]$name,[bool]$isolated,[string]$source=$publish){$out=Join-Path $sandbox $name;New-Item -ItemType Directory -Path $out | Out-Null;$args=@("/DPublishDir=$source","/O$out");if($isolated){$args+="/DTestRoot=$sandbox"};$args+=Join-Path $root 'installer/QLCAD.iss';& $Compiler @args *> (Join-Path $ResultDir ($name+'.log'));if($LASTEXITCODE -ne 0){throw "Compile failed: $name"};$built=(Get-ChildItem -LiteralPath $out -Filter '*.exe' | Select-Object -First 1).FullName; & (Join-Path $root 'tools/Verify-ExecutableIcon.ps1') -ExecutablePath $built | Out-Host; return $built}
...
$releaseBefore=Hash (Join-Path $root 'release/QLCAD.exe')
...
try {
 ...
 Check (-not(Test-Path -LiteralPath (Join-Path $escaped 'QLCAD.exe'))) 'destination override rejected'
 ...
 while((Test-Path -LiteralPath (Join-Path $install 'QLCAD.exe')) -and (Get-Date) -lt $deadline){Start-Sleep -Milliseconds 200}
 Check (-not(Test-Path -LiteralPath (Join-Path $install 'QLCAD.exe'))) 'clean uninstall removes executable'
 ...
 if ($PreviousPublishDir) {
   ...
   Check ((Hash (Join-Path $previous 'QLCAD.exe')) -eq $oldInfo.sha256) 'previous build executable matches metadata'
   ...
   Check ((Hash (Join-Path $install 'QLCAD.exe')) -eq $oldInfo.sha256) 'previous build installed before upgrade'
 ...
 Check ((Hash (Join-Path $install 'QLCAD.exe')) -eq $info.sha256) 'installed executable matches verified candidate'
 ...
 $deadline=(Get-Date).AddSeconds(10)
 while((Test-Path -LiteralPath (Join-Path $install 'QLCAD.exe')) -and (Get-Date) -lt $deadline){Start-Sleep -Milliseconds 200}
 foreach($relative in @('QLCAD.exe','CadPlugin/DwgTranslator.Cad.dll','CadPlugin/DwgTranslator.Core.dll','assets/default-glossaries/mechanical_zh_en.json')){Check (-not(Test-Path -LiteralPath (Join-Path $install $relative))) "uninstall removes owned $relative"}
 ...
 Check ($releaseBefore -eq (Hash (Join-Path $root 'release/QLCAD.exe'))) 'canonical release unchanged'
